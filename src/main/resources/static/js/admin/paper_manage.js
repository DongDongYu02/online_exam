 let paperTable = new Vue({
    ...
    methods: {
        ...
        //点击修改按钮打开修改试卷信息模态框，获取专业列表并回显该市卷的信息
        update: function (id) {
        ...
        getPageInfo: function (pageNum) {
            axios.get('paperInfo/operate?pageNum=' + pageNum).then((response) => {
        ...
        getPaper: function (id) {
            axios.get('paperInfo/operate/' + id).then((response) => {
                if (response.data.success === true) {
                    this.updatePaperInfo.papName = response.data.element.papName
                    this.updatePaperInfo.papMajor = response.data.element.papMajor
        ...
        //ajax请求添加试卷接口
        addPaper: function () {
            if(this.checkNewPapName() && this.checkNewPapMajor() && this.checkNewStartTime() && this.checkNewEndTime() && this.checkNewExamTime()){
                let value = this.$refs['newPapNameRef'].value;
                axios.get('paperInfo/checkPaperName?papName=' + value).then((response) => {
                  if(response.data.success){
                      axios.post('paperInfo/operate?' +
                          'papName=' + this.newPaperInfo.papName +
                          '&papMajor=' + this.newPaperInfo.papMajor +
        ...
        updateAxios:function (){
            axios.put('paperInfo/operate?' +
                'papName=' + this.updatePaperInfo.papName +
                '&papMajor=' + this.updatePaperInfo.papMajor +
        ...
        deletePaper(id) {
            axios.delete('paperInfo/operate/' + id).then((response) => {
        ...
        //新增所属专业输入框校验
        checkNewPapMajor: function () {
            let value = this.$refs['newPapMajorRef'].value;
            if (value === '') {
                this.newPapMajorHasError = true
                this.newPapMajorErrorMsg = '请选择试卷所属专业'
                return false
            }
            this.newPapMajorHasError = false
            this.newPapMajorErrorMsg = ''
